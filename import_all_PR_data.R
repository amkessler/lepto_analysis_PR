...
pr_causesbyyear_wide$mean.val <- round(mean_val,2)
pr_causesbyyear_wide$max.val <- max_val
pr_causesbyyear_wide$min.val <- min_val


### bring in code lookup file

code_lookup <- read_fwf("icd10cm_codes_2018.txt", 
                   fwf_widths(c(7, 1, 250), 
                   c("cause.of.death.icd10", "blank", "description"
                   )))

#join tables
joined <- left_join(pr_causesbyyear_wide, code_lookup)
joined$blank <- NULL
...
write.csv(ten_or_more, "test.csv")
